...
public class KThread {
	...
	//Join Implementation Variables
		//Threads Marked for Wakeup
	private static ArrayList<KThread> MarkedForWake;
		//Threads marked for Join
	private static ArrayList<KThread> MarkedForJoin;
    /**
	...
	MarkedForJoin = new ArrayList<KThread>();
	MarkedForWake= new ArrayList<KThread>();
    }
	...
	boolean interruption_status = Machine.interrupt().disable();
	...
	Machine.interrupt().restore(interruption_status);
    }
    ...
    public void join(){
	Lib.debug(dbgThread, "Target Join: "+toString());
	Lib.assertTrue(this!=currentThread); // Thread can't terminate itself

	boolean interruption_status = Machine.interrupt().disable();

	//Current thread must be running else restore the interruptions and return
	if(this.status == statusFinished) {
		Machine.interrupt().restore(interruption_status);
		return;
	}

	//Test if this thread has already been marked for join
	for(int i = 0 ; i < MarkedForJoin.size() ; i++){
		if(currentThread.compareTo( MarkedForJoin.get(i) ) == 0){
			Machine.interrupt().restore(interruption_status);
			return;
		}
	}
